...
@section('title', 'Créer Facture Fournisseur')
...
<form action="{{ route('fournisseur_factures.store') }}" method="POST">
    @csrf
                            ...
                            <div class="input-group">
                                ...
                                <select name="fournisseur_id" id="fournisseur_id" class="form-select @error('fournisseur_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($fournisseurs as $fournisseur)
                                        <option value="{{ $fournisseur->id }}" {{ (old('fournisseur_id') ?? session('duplicateData.fournisseur_id')) == $fournisseur->id ? 'selected' : '' }}>
                                            {{ $fournisseur->nom }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="date" 
                                       ...
                                       id="date_facture" 
                                       name="date_facture" 
                                       value="{{ old('date_facture', session('duplicateData.date_facture', date('Y-m-d'))) }}"
                                       required>
                                @error('date_facture')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <tbody>
                                ...
                                @if(count($duplicateArticles) > 0)
                                    @foreach($duplicateArticles as $article)
                                        ...
                                        <tr>
                                            <td class="py-2">
                                                <select name="articles[{{ $rowIndex - 1 }}][id]" class="form-select articleSelect" required>
                                                    <option value="">-- Sélectionner --</option>
                                                    @foreach($articles as $art)
                                                        <option value="{{ $art->id }}"
                                                                data-prix="{{ $art->prix }}"
                                                                data-tva="{{ $art->tva }}"
                                                                {{ $art->id == $article['id'] ? 'selected' : '' }}>
                                                            {{ $art->designation }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td class="py-2">
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" name="articles[{{ $rowIndex - 1 }}][prix]" class="form-control prix" value="{{ $articleData->prix ?? '' }}">
                                                </div>
                                            </td>
                                            <td class="py-2">
                                                <div class="input-group input-group-sm">
                                                    <input type="text" name="articles[{{ $rowIndex - 1 }}][tva]" class="form-control tva" readonly value="{{ $articleData->tva ?? '' }}">
                                            ...
                                            <td class="py-2">
                                                <div class="input-group input-group-sm">
                                                    <input type="text" name="articles[{{ $rowIndex - 1 }}][total]" class="form-control total" readonly value="">
                                                    <span class="input-group-text">DH</span>
                                                </div>
                                            </td>
                                            <td class="py-2">
                                ...
                                @else
                                    <tr>
                                        ...
                                        <td class="py-2">
                                            <div class="input-group input-group-sm">
                                                <input type="text" name="articles[0][tva]" class="form-control tva" readonly>
                                                <span class="input-group-text">%</span>
                                            </div>
                                        </td>
                                        <td class="py-2">
                                        ...
                                        <td class="py-2">
                                            <div class="input-group input-group-sm">
                                                <input type="text" name="articles[0][total]" class="form-control total" readonly>
                                                <span class="input-group-text">DH</span>
                                            </div>
                                        </td>
                                        <td class="py-2">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-ciel">
                            <i class="bi bi-check-lg me-1"></i> Créer Achat
                        </button>
                        <a href="{{ route('fournisseur_factures.index') }}" class="btn btn-outline-secondary">
                            Annuler
                        </a>
...
<script>
let rowIndex = {{ max(1, count(session('duplicateData.articles', []))) }};
